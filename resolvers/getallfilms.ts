...
export default async function getallFilms(req:Request,res:Response){

const series= await ModeloSerieopeli.find({tipo:"film"})
if(!series){
    res.status(403).send("No se han encontrado peliculas");
    return;
}
